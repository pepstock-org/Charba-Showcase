...
//		chart.getOptions().getHover().setIntersect(true);
		...
		axis2.getTicks().setCallback(new TickCallback() {
			...
			public String onCallback(AbstractChart<?, ?> chart, TickItem item) {
				...
				Dataset ds = dss.get(item.getIndex());
				return ds.getLabel();
			}
		});
...
//		chart.getData().setDatasets(dataset1, dataset2);
		...
		AbstractPlugin p = new AbstractPlugin() {
			...
			public void onAfterDatasetsDraw(AbstractChart<?, ?> chart, double easing, JavaScriptObject options) {
				...
				ScaleItem scaleX = chart.getChartNode().getScales().getItems().get(ScalesNode.DEFAULT_X_AXIS_ID);
				ScaleItem scaleY = chart.getChartNode().getScales().getItems().get(ScalesNode.DEFAULT_Y_AXIS_ID);
